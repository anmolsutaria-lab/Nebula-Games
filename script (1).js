...
const gamesData = [
    ...
    { title: "1v1 LOL", cat: "Action", img: "https://amplify8.neocities.org/images/lol.jpeg", url: "https://script.google.com/macros/s/AKfycbxfAVCW_RMesBbzqklT1cpIvGqdNgXEQTBTEBx2D6SQGv3v8DvE_5i8Vbo1Fml8bMxw/exec" },
    ...
    { title: "Eaglercraft", cat: "Action", img: "https://amplify8.neocities.org/images/craft.jpg", url: "https://mysticgames.netlify.app/games/eaglercraft/dragonx_v4%20(1)" },
...
// === PANIC MODE (iframe-aware) ===
(function () {
    const PANIC_URL = "https://launchpad.classlink.com/olentangy";

    function triggerPanic() {
        if (document.getElementById('panic-toggle').checked) {
            window.location.replace(PANIC_URL);
        }
    }

    // Main window key handler — fires when focus is on the parent page
    ...
    // When a game iframe is open, it steals keyboard focus.
    // We place a thin transparent overlay div OVER the iframe that:
    //   1. Catches pointer events so we can detect clicks near the iframe
    //   2. Uses a periodic focus-reclaim loop to pull focus back to parent
    //      the moment the user isn't actively interacting with the game
    // We also inject a postMessage listener — if somehow the iframe sends
    // a message we relay to panic (future-proofing), but mainly we use
    ...
    // Key listener attached to the overlay strip
    overlay.addEventListener('keydown', handleKey);

    // Also: poll to reclaim focus every 800ms while a game is open.
    // This is the key fix — browsers allow window.focus() from the top frame.
    let panicInterval = null;

    function startFocusReclaim() {
        overlay.style.display = 'block';
        panicInterval = setInterval(() => {
            // If the active element is the iframe, quietly refocus the window
            // so our keydown listener can fire again
    ...
    // Hook into launchGame / closeGame to start/stop the reclaim loop
    ...
    const origClose = window.closeGame;
